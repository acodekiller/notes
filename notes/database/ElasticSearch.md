...
### 5）FOR(Frame of Reference)  帧编码

**FOR 和 RBM 都是作用于倒排索引中的 Posting List（文档 ID 列表）的压缩算法。**

#### 1. 核心思想

不存原始 ID，存差值（增量），然后用最少的位数存这些差值。

#### 2. 工作原理

假设有 ID 列表：`[1, 2, 3, 5, 100, 101, 102]`
...
- Lucene 会将数据分块（例如每 128 个或 256 个 ID 一组）。
- 块1：`[1, 1, 1, 2]`，最大增量是 2，需要 **2 位 (bit)** 表示。
- 块2：`[95, 1, 1]`，最大增量是 95，需要 **7 位 (bit)** 表示。

**第三步：按位存储**

- 块1：4 个数 × 2 位 = 8 位 = **1 字节**。
- 块2：3 个数 × 7 位 = 21 位 = **3 字节**。

**压缩结果**：

- 原始大小：7 个数 × 4 字节 = 28 字节。
- FOR 压缩后：1 字节 + 3 字节 = **4 字节** + 块头。
- 压缩比约 **7:1**。

#### 3. 总结

- **优点**：压缩比极高，速度也很快（解压时只需按位读取并累加）。
- **缺点**：如果数据稀疏（如 `[1, 10000, 20000]`），差值会很大，压缩效果差，甚至可能膨胀。
- **适用**：文档 ID **密集、连续**的场景（例如几乎所有查询都会遍历的大范围数据）。

### 6）RBM (Roaring Bitmap)  咆哮位图

#### 1. 核心思想

将 32 位的文档 ID 拆成两半：高 16 位和低 16 位。高 16 位用来确定“桶”，低 16 位用来在桶里做位图（或数组）。

#### 2. 工作原理

假设有 ID 列表：`[1000, 10000, 100000, 1000000]`

**第一步：拆分为高 16 位和低 16 位**

- 一个 int 是 32 位，高 16 位代表“桶编号”，低 16 位代表“桶内偏移”。

| 文档 ID | 高 16 位 (桶) | 低 16 位 (偏移) | 原理                                          |
| :------ | :------------ | :-------------- | --------------------------------------------- |
| 1000    | 0             | 1000            | (0 << 16) + 1000 = 1000                       |
| 10000   | 0             | 10000           | (0 << 16) + 10000 = 10000                     |
| 100000  | 1             | 34464           | (1 << 16) + 34464 = 65536 + 34464 = 100000    |
| 1000000 | 15            | 16960           | (15 << 16) + 16960 = 983040 + 16960 = 1000000 |
...
RBM 会根据桶内数据的密度，自动选择最高效的存储方式：

- **Array Container (数组容器)**：当桶内 ID 数量 < 4096 时，用一个 `short[]` 数组存储。空间小，适合稀疏数据。
  - 桶 0（2 个 ID）、桶 1（1 个 ID）、桶 15（1 个 ID） → 用数组。
- **Bitmap Container (位图容器)**：当桶内 ID 数量 ≥ 4096 时，用一个 `65536` 位的固定长度位图（`8KB`）存储。无论有多少个 ID，都只占 8KB。查询极快（位运算），适合密集数据。

#### 3. 总结

- **优点**：压缩率高（尤其对稀疏数据），且**支持快速的位运算**（AND、OR），这对于多条件查询合并倒排列表至关重要。
- **缺点**：对于极其密集的数据，位图容器固定占 8KB，不如 FOR 灵活。
- **适用**：文档 ID **稀疏、跳跃大**的场景。

> #### FOR 和 RBM对比
>
> **Lucene 会自动选择最优方案**。简单来说：
>
> 1. **默认首选 FOR**：因为大多数 ID 在段（Segment）内是连续或接近连续的。
> 2. **当差值过大或数据稀疏时**：自动切换为 **RBM** 存储。
>
> 这种**自适应**策略，让 ES 既能享受 FOR 的高压缩比，又能利用 RBM 的快速合并能力。这两种算法协同工作，是 ES 能够兼顾低磁盘占用和高查询性能的关键技术之一
...
- **TF (Term Frequency)**：词频，某个词在文档中出现的次数。出现次数越多，该词对该文档的重要性越大。
- **IDF (Inverse Document Frequency)**：逆文档频率，用于衡量一个词在所有文档中有多少重要度。出现频率越低，IDF 越高，反之亦然。
- **Field Length Norm（规范化字段长度）**：字段的长度。一般来说，较短的字段如果包含查询词，那么该字段的相关性得分会更高。
...
| 因素                  | TF-IDF                          | BM25                         | 说明                          |
| :-------------------- | :------------------------------ | :--------------------------- | :---------------------------- |
| **TF（词频）**        | 线性增长                        | **饱和增长**                 | BM25 对词频做了边际递减处理   |
| **IDF（逆文档频率）** | `log(N / n)`                    | `log(1 + (N-n+0.5)/(n+0.5))` | BM25 的 IDF 公式略有优化      |
| **字段长度归一化**    | 朴素除法（字段长度 / 平均长度） | **可调参数 b 控制**          | BM25 可以控制长度归一化的强度 |
      ...
      "functions": [
        {
          "gauss": {
            "publish_date": {
              "origin": "now",		//表示时间衰减的起点是当前时间
              "scale": "14d",		//表示14天是一个时间段，在这个时间段内评分将显著衰减
              "decay": 0.5			//0.5 表示在14天后评分将衰减到原来的50%
            }
          }
        }
      ],
...
- 改变衰减速度：通过调整 "scale" 和 "decay" 参数，可以控制衰减的速度和衰减后的评分。例如，如果希望评分衰减得更快，可以缩短 "scale"（比如 "scale": "7d"）或增加 "decay" 值（比如 "decay": 0.3"）。
...
## 8、如何通过 Elasticsearch 的 Watcher 来实现基于条件触发的报警系统？
...
通过 __watcher/watch/{watch_id} API 创建一个新的 Watch。下面提供一个示例 Watch 配置：
...
利用 ES 文档 `_id` 的唯一性约束。当多个客户端同时尝试创建同一个 `_id` 的文档时，**只有第一个能成功（HTTP 201）**，其余都会收到冲突错误（HTTP 409）。

**核心机制**：

- 锁的 key 映射为文档 `_id`
- 锁持有者信息存储在文档 `_source` 中
- 必须设置 `expire_at` 过期时间作为兜底

#### 2. 示例：

```json
// 获取锁（只有第一次能成功）
PUT /locks/_doc/my_lock/_create
{
  "holder": "service-1",
  "acquire_time": "2025-01-15T10:00:00Z",
  "expire_at": "2025-01-15T10:00:30Z"
}

// 续期（直接更新，无需版本校验）
POST /locks/_doc/my_lock/_update
{
  "doc": { "expire_at": "2025-01-15T10:01:00Z" }
}

// 释放锁（直接删除）
DELETE /locks/_doc/my_lock
```

#### 3. 优点

1. 实现简单：只需要一次`_create`操作，成功/失败结果明确；
2. 原子性强：`_create`本身是原子的，无竞争条件；
3. 故障容错性好：主分片故障转移后，`_id`唯一性约束依然有效；
4. 续期/释放简单：无需版本校验，直接更新或删除即可

#### 4. 缺点

必须设置 TTL：需要业务层实现过期检查和清理，否则可能产生"僵尸锁"

过期锁接管需额外逻辑：需要先检查 `expire_at`，再尝试接管，存在极小的时间窗口

性能一般：每次操作都是写入，有磁盘 I/O

### 2）_version（乐观锁）

#### 1. 实现原理

ES 中每个文档都有一个 `_version` 字段，每次更新（包括删除）都会使版本号 `+1`。利用条件更新 `?version=xxx`，只有当文档当前版本号与指定版本号一致时，更新才会成功。

**核心机制**：

- 先 `GET` 查询锁文档，获取当前 `_version`
- 再 `PUT` 更新锁文档，携带 `?version=当前版本号`
- 版本匹配则更新成功（获得锁），否则失败
...
```json
// 1. 查询锁，获取当前版本号
GET /locks/_doc/my_lock
// 返回：{ "_version": 5, ... }

// 2. 尝试获取锁（条件更新）
PUT /locks/_doc/my_lock?version=5
{
  "holder": "service-1",
  "acquire_time": "2025-01-15T10:00:00Z"
}
...
// 释放锁（也需要版本校验）
DELETE /locks/_doc/my_lock?version=6
```

#### 3. 优点

1. 实现直观：版本号的概念容易理解
2. 支持重入：可以通过版本号记录重入次数

#### 4. 缺点

| 缺点             | 说明                                              |
| :--------------- | :------------------------------------------------ |
| **非原子操作**   | 需要 `GET` + `PUT` 两步，存在竞态条件窗口         |
| **故障容错差**   | 主分片故障转移后，`_version` 可能重置，导致锁混乱 |
| **释放锁需版本** | 如果忘记版本号或版本已变，无法释放                |
| **已废弃趋势**   | ES 官方逐渐推荐 `_seq_no + _primary_term`         |

### 3）_seq_no + _primary_term

#### 1. 实现原理

ES 7.x+ 引入的乐观锁机制，比 `_version` 更精确：

- `_seq_no`：文档级别的**全局递增序列号**，每次修改 `+1`
- `_primary_term`：主分片的**世代号**，每次主分片重新选举 `+1`

**核心机制**：

- `_seq_no` 检测并发修改
- `_primary_term` 检测主分片是否被重置
- 两者组合，提供更强的并发控制

#### 2. 示例

```json
// 1. 查询锁，获取 _seq_no 和 _primary_term
GET /locks/_doc/my_lock
// 返回：{ "_seq_no": 10, "_primary_term": 1, ... }

// 2. 尝试获取锁（条件更新）
PUT /locks/_doc/my_lock?if_seq_no=10&if_primary_term=1
{
  "holder": "service-1",
  "acquire_time": "2025-01-15T10:00:00Z"
}
...
// 释放锁（条件删除）
DELETE /locks/_doc/my_lock?if_seq_no=11&if_primary_term=1
```
...
| 优点               | 说明                                             |
| :----------------- | :----------------------------------------------- |
| **最精确**         | `_seq_no` + `_primary_term` 双重校验，安全性最高 |
| **检测主分片重置** | `_primary_term` 变化时操作必然失败，避免"僵尸锁" |
| **官方推荐**       | ES 7.x+ 官方推荐使用此方式替代 `_version`        |

#### 4. 缺点

| 缺点                 | 说明                                                         |
| :------------------- | :----------------------------------------------------------- |
| **非原子操作**       | 同样需要 `GET` + `PUT` 两步（可优化为 `_update` + `upsert`） |
| **实现复杂**         | 需要处理 `_primary_term` 不匹配的异常场景                    |
| **故障转移后锁失效** | 主分片切换后，基于旧 `_seq_no` 的续期/释放必然失败（需要 TTL 兜底） |

### 4）对比

| 场景                 | `_create`                          | `_version`                      | `_seq_no + _primary_term`               |
| :------------------- | :--------------------------------- | :------------------------------ | :-------------------------------------- |
| **主分片故障转移**   | 锁文档仍存在，`_id` 唯一性依然有效 | `_version` 可能重置，锁状态混乱 | `_primary_term` 变化，续期/释放必然失败 |
| **锁持有者续期**     | 直接更新，成功 ✅                   | 版本不匹配，失败 ❌              | `_primary_term` 不匹配，失败 ❌          |
| **锁持有者释放**     | 直接删除，成功 ✅                   | 版本不匹配，失败 ❌              | `_primary_term` 不匹配，失败 ❌          |
| **其他客户端获取锁** | `_create` 冲突，失败 ❌             | 可能成功（如果版本重置）⚠️       | 可能成功（但旧锁残留）⚠️                 |

**结论**：只有 `_create` 方案在故障转移后仍然能保持锁的基本语义（已持有的锁依然有效，其他客户端无法抢占）。

> 建议：
>
> | 场景                 | 推荐方案                  | 原因                   |
> | :------------------- | :------------------------ | :--------------------- |
> | **生产环境（通用）** | ✅ **`_create`**           | 简单、可靠、故障容错好 |
> | **强一致性场景**     | `_seq_no + _primary_term` | 检测主分片重置，更精确 |
> | **高频锁操作**       | ❌ 不要用 ES               | 改用 Redis             |
> | **强一致性+高可用**  | ❌ 不要用 ES               | 改用 ZooKeeper/etcd    |

## 10、如何在 Elasticsearch 中处理大规模数据的去重操作？

### 1）cardinality（统计唯一值数量）

**适用场景**：只需要知道有多少个不同的值，不需要具体值列表。
...
GET /orders/_search
{
  ...
  "aggs": {
    "unique_users": {
      "cardinality": {
        "field": "user_id",
        "precision_threshold": 40000  // 精度阈值（内存与精度的平衡）
      }
    }
  }
}
...
- 基于 HyperLogLog++ 算法，误差约 0.5%
- `precision_threshold` 越高越精确，但内存消耗越大（建议 40000 以内）
- 性能极快，适合百亿级数据

### 2）terms 聚合（获取去重后的值列表）- 小规模

**适用场景**：需要获取去重后的具体值（如所有不重复的用户 ID）。
...
GET /orders/_search
{
  ...
  "aggs": {
    "distinct_users": {
      "terms": {
        "field": "user_id",
        "size": 10000,           // 最多返回多少个唯一值
        "shard_size": 50000      // 每个分片返回的数量（建议大于 size）
      }
    }
  }
}
...
- `size` 最大受 `index.max_result_window` 限制（默认 10000）
- 如果唯一值超过 10000，需要分页或改用其他方案

### 3）composite 聚合（获取去重后的值列表）- 中大规模

**适用场景**：唯一值数量 > 10000，需要分批获取全部去重结果。
  ...
  "aggs": {
    "distinct_users": {
      "composite": {
        ...
        "after": { "user_id": "user_1000" }   // 从上一次的最后继续
      }
    }
  }
}
...
### 4）collapse 字段折叠（去重返回文档）

**适用场景**：需要返回去重后的文档详情（如每个用户的最新一条订单）。
...
- 只返回每个组的第一条文档（可控制排序）
- 比 `terms + top_hits` 性能更好
- 适合"去重后展示"的场景

### 5）top_hits 聚合（获取每组代表文档）

**适用场景**：需要按多个字段组合去重，或需要每组的多条文档。
      ...
      "aggs": {
        "latest_order": {
          "top_hits": {
            "size": 1,
            "sort": [{ "order_date": "desc" }],
            "_source": ["user_id", "order_date", "amount"]
          }
        }
      }
    }
...
## 11、FieldData & doc_values 数据结构

Doc Values 和 Fielddata 都是 Elasticsearch 中用于**排序、聚合和脚本访问**的数据结构，但它们的**设计哲学、存储位置、性能特征**完全不同。下面是完整的对比总结。

### 1）概念

- **Doc Values**：索引时预先构建的、存储在**磁盘**上的列式存储，是 ES 高性能排序/聚合的**默认方案**
- **Fielddata**：查询时动态构建的、存储在**JVM堆内存**中的数据结构，是 `text` 字段排序/聚合的**最后手段**

| 维度             | Doc Values                              | Fielddata                    |
| :--------------- | :-------------------------------------- | :--------------------------- |
| **存储位置**     | **磁盘**（通过 OS 缓存加速）            | **JVM 堆内存**               |
| **构建时机**     | **索引时**（预先构建）                  | **查询时**（动态构建）       |
| **适用字段**     | `keyword`、数值、日期、`text`（可开启） | `text`（需显式开启）         |
| **默认状态**     | ✅ **开启**（keyword/数值/日期）         | ❌ **关闭**                   |
| **构建成本**     | 写入时一次性成本                        | 查询时实时成本（可能很慢）   |
| **内存占用**     | 低（通过 OS page cache）                | **高**（堆内存）             |
| **GC 影响**      | 无                                      | **严重**（可能触发 Full GC） |
| **首次查询速度** | 快（已构建好）                          | 慢（需动态构建）             |
| **磁盘占用**     | 额外 10-20%                             | 无                           |
| **压缩技术**     | 字典编码、位压缩、RLE                   | 无（原始数据）               |
| **推荐程度**     | ✅ **强烈推荐**                          | ❌ 不推荐（除非绝对必要）     |

### 2）工作原理

- Doc Values:

![image-20260420202647038](imgs/image-20260420202647038.png)

- Fielddata：

![image-20260420202715666](imgs/image-20260420202715666.png)

### 3）总结

Doc Values 和 Fielddata 都是 ES 中用于排序和聚合的数据结构，但设计完全不同：

**Doc Values** 是**索引时**在**磁盘**上构建的列式存储，是 ES 的默认方案。它内存友好（通过 OS 缓存），无 GC 压力，性能稳定。`keyword`、数值、日期类型默认开启 Doc Values。

**Fielddata** 是**查询时**在**JVM 堆内存**中动态构建的数据结构，用于支持 `text` 字段的排序和聚合。它内存占用大，会引发 GC，首次查询慢，默认关闭。

**最佳实践**：

- 永远优先使用 Doc Values
- 如果需要对 `text` 字段排序/聚合，添加 `keyword` 子字段，不要开启 `Fielddata`
- 不需要排序/聚合的字段，可以关闭 Doc Values 节省磁盘
- Fielddata 只作为最后手段，且必须限制内存大小
