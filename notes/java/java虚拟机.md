...
![image-20260520141717969](imgs/image-20260520141717969.png)

### 1）类加载器子系统

类加载器子系统负责将 `.class` 文件加载到 JVM 中，把 Java 类从文件系统或网络中加载进来，转化为 JVM 能理解的数据结构。

类加载器的主要过程分三步走：

1）加载阶段，找到类文件并读取字节流，在方法区创建对应的类元数据

2）链接阶段又细分三步：验证字节码格式是否合规、准备阶段给静态变量分配内存并设置默认值、解析阶段把符号引用转换成直接引用

3）初始化阶段，执行类的静态代码块和静态变量赋值，这时候才真正跑用户写的初始化逻辑

类加载采用双亲委派模型，加载请求先往上抛给父加载器，父加载器确定不自己动手。这样能避免核心类被篡改，比如你自己写个 java.lang.String 是加载不进去的。

### 2）运行时数据区域

运行时数据区是 JVM 内存管理的核心，按照线程是否共享分两类：

线程私有的区域：
1）程序计数器，记录当前线程执行到哪条字节码指令了，是唯一不会 OOM 的区域
2）虚拟机栈，每个方法调用会压一个栈帧进去，存局部变量表、操作数栈、方法出口这些信息
3）本地方法栈，跟虚拟机栈差不多，专门给 native 方法用的

线程共享的区域：
1）堆，对象实例基本都在这儿分配，也是 GC 的主战场，按分代模型分年轻代和老年代
2）方法区，存类的元数据、常量池、静态变量。JDK 8 之前叫永久代，8 之后改成 Metaspace，直接用本地内存

![image-20260520142115103](imgs/image-20260520142115103.png)

### 3）执行引擎

执行引擎负责把字节码转换为机器指令并执行，内部有两个核心组件配合工作：

- 解释器负责逐条解释字节码并执行，启动速度快但效率一般。
- JIT 编译器会把执行次数超过阈值的热点代码编译成本地机器码缓存起来，下次直接执行机器码，性能提升明显。
...
声明一个 native 方法后，用 javah 生成头文件，再用 C/C++ 实现对应函数，编译成动态链接库，Java 里 System.loadLibrary 加载进来就能调用了。

## 2、Java是如何实现跨平台的

Java 跨平台的核心就是 JVM。Java 源码编译后生成的不是特定操作系统的机器码，而是一种中间格式的字节码（.class 文件）。

不同操作系统上都有各自实现的 JVM，字节码丢给 JVM，JVM 负责把它翻译成当前平台能认识的机器指令。所以同一份 .class 文件，拿到 Windows 上能跑，拿到 Linux 上也能跑，拿到 macOS 上照样能跑。

![image-20260520174907940](imgs/image-20260520174907940.png)

到 Oracle 官网下载 JDK 会发现分了很多版本：Windows x64、Linux x64、Linux ARM、macOS x64、macOS ARM 等等。这说明 JVM 本身是平台相关的，针对每个平台都有专门的实现。

![image-20260520175019631](imgs/image-20260520175019631.png)

JVM 底层用 C/C++ 写的，它要跟操作系统打交道，处理内存分配、线程调度、文件 IO 这些脏活累活。所以 JVM 开发者替我们把平台差异吃掉了，我们写的 Java 代码才能做到一次编写到处运行。

> 有些场景必须调用操作系统底层能力，比如操作硬件、调用现有的 C 库。Java 提供了 JNI（Java Native Interface）让 Java 代码能调用 C/C++ 写的本地方法。
>
> 一旦用了 JNI，跨平台就打折扣了，因为本地库得针对每个平台单独编译。所以能不用 JNI 就别用，除非确实有性能瓶颈或者必须调用系统底层 API。
>
...
JIT 就是 JVM 在运行时把字节码编译成机器码的技术。Java 程序刚启动时是解释执行的，一行一行翻译字节码，速度比较慢。但 JVM 会统计每个方法被调用的次数，当某段代码被执行超过一定阈值，JVM 就认为它是热点代码，把它编译成本地机器码缓存起来，下次直接执行机器码，性能能提升 10 倍以上。

举个例子，一个方法被调用了 10000 次，前几千次是解释执行，JIT 发现这是热点后编译成机器码，后面几千次就直接跑编译好的代码了。所以 Java 程序有个“预热”的过程，跑一段时间后性能会明显上来。

![image-20260528112228218](imgs/image-20260528112228218.png)
...
这两个计数器可以通过 -xx:compileThreshold 调整。如果你的服务启动后马上要打流量，可以把阈值调低让 JIT 早点介入；如果更看重启动速度，可以调高阈值减少编译开销。
...
1) 方法内联：把被调用方法的代码直接嵌入调用处，省掉方法调用的栈开销。比如 getter/setter 这种小方法，内联后跟直接访问字段一样快。

2) 逃逸分析：分析对象的作用域，如果一个对象只在方法内部使用，不会逃逸到外部，JIT 可以把它分配在栈上而不是堆上，省掉 GC 压力；甚至直接把对象拆成几个基本类型变量，这叫标量替换。

3) 循环展开：把循环体复制多份减少循环判断次数。比如原来循环 100 次，展开后变成循环 25 次但每次执行 4 份代码，减少了 75 次循环判断。

4) 空值检查消除：如果 JIT 分析出某个引用不可能为 null，就把 null 检查的指令删除。

![image-20260528113111241](imgs/image-20260528113111241.png)

[逃逸分析](/notes/java/java虚拟机/逃逸分析)
...
-XX:TieredStopAtLevel=1        # 只用 C1，追求快速启动
-XX:-TieredCompilation         # 关闭分层编译，只用 C2
-XX:+PrintInlining             # 打印内联决策
```

如果你的服务是 Serverless 场景，冷启动很频繁，可以考虑关掉 C2 只用 C1，牺牲峰值性能换取更快的启动。如果是长期运行的后端服务，保持默认的分层编译就行。

### 6）常见问题

**提问：JIT 编译是在主线程序执行的吗？会不会阻塞业务代码？**

回答：不是。JIT 编译在独立的后台线程执行，不会阻塞业务代码。当某个方法达到编译阈值时，JVM 把编译任务丢给编译线程，业务代码继续解释执行；等编译完成后，下次调用这个方法时才会切换到编译后的机器码。所以从业务角度看，JIT 编译是无感知的，最多就是 CPU 占用会高一些。

**提问：为什么 Java 程序需要“预热”？预热多久算够？**

回答：预热是为了让 JIT 把热点代码编译好，让 JVM 的各种缓存都填满。刚启动时解释执行慢，JIT 还没介入，内联缓存也是空的。预热时间取决于业务复杂度，简单服务几秒就够，复杂服务可能要几分钟。判断标准是看 RT 和吞吐量是否稳定下来，或者看 -XX:+PrintCompilation 输出的编译活动是否明显减少。生产环境可以用 Arthas 的 jad 命令查看某个方法是否已经被编译。

**提问：JIT 编译后的代码有没有可能被丢掉重新编译？**

回答：有。这种情况叫反优化。JIT 做优化时会基于当前的运行情况做假设，比如假设某个分支永远不会走到、某个类型检查永远不是 true。如果后续运行打破了这些假设，JVM 就会把之前编译的代码作废，退回到解释执行，然后基于新的信息重新编译。最常见的场景是类层次结构变化，比如之前只有一个实现类所以 JIT 做了激进内联，后来加载了新的子类，之前的假设就不成立了。

**提问：Graal 编译器和 C2 有什么区别？为什么有人说它是未来？**

回答：Graal 是用 Java 写的 JIT 编译器，C2 是 C++ 写的。用 Java 写的好处是更容易维护、更容易做实验性优化。性能上 Graal 在某些场景能比 C2 快 10%~20%，尤其是 Scala 这种大量使用高阶函数的语言。另外 Graal 是 GraalVM 的核心组件，支持 AOT 编译成 native image，冷启动时间能从秒级降到毫秒级，这对 Serverless 场景非常有吸引力。
...
AOT 最大的好处是启动快。传统 Java 程序启动要加载几千个类、JIT 要预热，一个 Spring Boot 应用启动可能要好几秒。用 GraalVM 编译 native image 后，启动时间能从 5 秒降到 50 毫秒，内存占用也能从 200MB 降到 30MB。这对 Serverless、云函数这种冷启动敏感的场景非常有吸引力。
...
![image-20260528151538262](imgs/image-20260528151538262.png)
...
编译出来的文件就是一个普通的可执行文件，双击就能跑，跟GO、Rust编译出来的一样。
...
![image-20260528152411533](imgs/image-20260528152411533.png)
...
优点：JVM在内存新生代EdenSpace中开辟了一小块区域，由线程私有，称作TLAB（Thread-localallocationbuffer），默认设定为占用**EdenSpace的1%**。在Java程序中很多对象都是小对象且用过即丢，它们不存在线程共享也适合被快速GC，所以对于小对象通常JVM会优先分配在TLAB上，并且TLAB上的分配由于是线程私有所以没有锁开销。因此在实践中分配多个小对象的效率通常比分配一个大对象的效率要高。
...
![image-20260528170337596](imgs/image-20260528170337596.png)
...
> **提问：Full GC 和 Major GC 是一回事吗？**
>
> 回答：严格来说不是。Major GC 只清理老年代，Full GC 是清理整个堆加元空间。但实际上很多人混用这两个概念，不同收集器的 GC 日志命名也不统一。面试时如果被追问，最好先确认对方的定义。HotSpot 的官方文档里，Full GC 明确包含年轻代、老年代和元空间的回收。
...
PLAB（Promotion Local Allocation Buffer）是 GC 线程在晋升对象时用的本地缓冲区，思路跟 TLAB 一样：避免多线程抢同一块内存。

YGC 时存活对象要从新生代搬到 Survivor 区或老年代，多个 GC 线程并行干活就会抢老年代的 freelist。PLAB 的做法是让每个 GC 线程先从 freelist 申请一块空间当私有缓冲区，后续晋升对象直接在自己的 PLAB 里用指针碰撞分配，不用每次都去抢全局锁。

GC 线程从 freelist 申请一块 PLAB 空间，然后在 PLAB 内部通过指针碰撞分配晋升对象的内存。多个 GC 线程各自操作自己的 PLAB，互不干扰。

![image-20260529102508977](imgs/image-20260529102508977.png)

这个机制在 G1、ZGC、Parallel GC 里都有，只是细节实现不同。核心目标就一个：让 GC 线程并行晋升对象时不打架。

### 2）PLAB 和 TLAB 的区别

两者思想完全一样，都是线程本地缓冲避免锁竞争，但使用场景不同：

| 对比维度 | TLAB            | PLAB                |
| -------- | --------------- | ------------------- |
| 使用者   | 应用线程        | GC 线程             |
| 分配位置 | Eden 区         | Survivor 区或老年代 |
| 触发时机 | 应用创建新对象  | GC 晋升存活对象     |
| 内存来源 | Eden 区连续空间 | freelist 空闲链表   |

TLAB 是给应用线程在 Eden 区创建新对象的；PLAB 是给 GC 线程在 Survivor 或老年代安置存活对象用的。

### 3）PLAB 的动态调整机制

PLAB 大小不是写死的，JVM 会根据每次 GC 的实际情况动态调整。调整依据主要有两个：

1. 晋升对象的数量：如果这次 GC 晋升了大量对象，下次 PLAB 会给大一点
2. PLAB 浪费率：PLAB 用不完的空间就浪费了，如果浪费率太高会调小

### 4）PLAB 和晋升失败

如果 PLAB 分配不出空间，而且 freelist 也申请不到新的 PLAB 块，就会发生晋升失败（Promotion Failure）。这时候 G1 会触发一次 Full GC 来腾空间。

### 5）晋升失败的常见原因

1. 老年代碎片化严重，freelist 里全是小块凑不出大的 PLAB
2. 晋升速度太快，老年代空间不够用
3. Mixed GC 回收老年代的速度跟不上晋升速度

遇到频繁晋升失败可以考虑增大堆内存、调整 Mixed GC 触发阈值（-XX:InitializingHeapOccupancyPercent），或者排查是不是有大量长寿命对象一直晋升。

### 6）常见问题

**提问：PLAB 浪费的空间怎么处理？会不会造成老年代碎片？**

回答：PLAB 用剩的空间会被填充哑对象（dummy object），跟 TLAB 处理方式一样，保证内存可以线性遍历。至于碎片问题，G1 用的是 Region 机制，每个 Region 回收后整块释放，不存在传统意义上的碎片。但如果大对象频繁分配确实会造成 Humongous Region 的碎片化。

**提问：为什么 PLAB 从 freelist 分配而不是像 TLAB 那样用指针碰撞？**

回答：因为老年代内存不连续。经过多次 GC 之后老年代会有很多空洞，只能用 freelist 来管理这些零散的空闲块。TLAB 在 Eden 区分配，Eden 区每次 GC 后清空重来，内存是连续的，所以能用指针碰撞。

**提问：G1 的 GC 线程数量是怎么决定的？每个线程都有自己的 PLAB 吗？**

回答：G1 的并行 GC 线程数由 -XX:ParallelGCThreads 控制，CPU 核心数 N ≤ 8 时等于 N；当 N > 8 时等于 8 + (N-8) × 5/8。例如 4 核机器默认是 4 线程，16 核机器默认是 13 线程。每个参与 GC 的线程都有自己的 PLAB，互相不干扰。线程数太多反而可能造成 PLAB 浪费增加，因为每个线程的 PLAB 可能都用不满。
...
1. 检查是否已加载
        ↓
2. 如果有父类加载器 → 直接委派给父类（由父类完整执行它的 loadClass）
   - 父类又会检查自己是否已加载
   - 父类如果有它的父类，继续向上委派
   - 最终到达根类加载器（Bootstrap）
      ↓
...
[什么时候需要打破双亲委派机制](/notes/java/java虚拟机/打破双亲委派机制案例)
...
JVM 垃圾回收调优的两个核心目标是 **低延迟**和**高吞吐量**，这两个目标往往是互斥的，需要根据业务场景做取舍。

- 低延迟指的是每次 GC 停顿的时间要短，保证应用的响应能力。对于实时交易系统、在线游戏、即时通讯这类场景，用户对卡顿非常敏感，100ms 的停顿就可能导致订单超时或者体验变差。
- 高吞吐量指的是单位时间内完成更多的业务处理，GC 占用的 CPU 时间越少越好。对于离线计算、批处理任务、大数据分析这类场景，用户不在乎某一刻卡不卡，只关心整体跑完要多久。

> **为什么低延迟和高吞吐不可兼得**
>
> GC 要做的事情总量是固定的：扫描对象图、标记存活对象、回收垃圾内存。这些工作要么集中做，要么分散做。
>
> 集中做就是攒一波再回收，每次回收量大但频率低，总的 CPU 开销小，吞吐量高，但单次停顿时间长。
>
> 分散做就是少量多次，每次回收量小停顿短，但频率高，而且并发标记需要额外的 CPU 资源和内存屏障开销，总的 CPU 消耗反而更大。

### 2）不同垃圾收集器的目标定位

| 收集器     | 核心目标 | 典型停顿       | 适用场景           |
| ---------- | -------- | -------------- | ------------------ |
| Serial     | 简单稳定 | 几十到几百 ms  | 客户端、小型应用   |
| Parallel   | 高吞吐   | 几百 ms 到数秒 | 批处理、后台计算   |
| CMS        | 低延迟   | 几十 ms        | Web 服务（已废弃） |
| G1         | 平衡     | 50-200ms 可控  | 通用服务端         |
| ZGC        | 超低延迟 | <10ms          | 超大堆、金融交易   |
| Shenandoah | 超低延迟 | <10ms          | 类似 ZGC           |

JDK 8 默认是 Parallel，JDK 9 开始默认是 G1。如果对延迟有极致要求，JDK 11+ 可以用 ZGC。
...
用jstat监视虚拟机信息`jstat -gc [pid]50010`：每500毫秒打印一次Java堆状况（各个区的容量、使用容量、gc时间等信息），打印10次

### 3）jmap（MemoryMapforJava）

用jmap（MemoryMapforJava）查看堆内存信息执行`jmap -histo pid`可以打印出当前堆中所有每个类的实例数量和内存占用，如下，classname是每个类的类名（[B是byte类型，[C是char类型，[I是int类型），bytes是这个类的所有示例占用内存大小，instances是这个类的实例数量
...
说明新生代太小了，对象分配速度快，很快就把 Eden 填满了。解决方案是调大新生代，比如 -Xmn 或者 -XX:NewRatio。但也不能调太大，不然单次 Young GC 扫描时间会变长。

**对象过早晋升到老年代**

可能是 Survivor 区太小，Young GC 之后存活的对象 Survivor 装不下，只能直接进老年代。解决方案是调大 Survivor，用 -XX:SurvivorRatio 来调整 Eden 和 Survivor 的比例。

也可能是晋升阈值 -XX:MaxTenuringThreshold 设得太小，对象还没来得及被回收就晋升了。默认值是 15，如果设成 6 那对象经历 6 次 Young GC 就会晋升。

**Full GC 频繁**

先排查是不是内存泄漏，用 MAT 或者 jmap 看看老年代里都是啥对象。如果不是泄漏，那就是老年代增长太快，要么是上面说的晋升太快，要么是大对象直接进老年代。

大对象阈值用 -XX:PretenureSizeThreshold 控制（只对 Serial 和 ParNew 生效），超过这个大小的对象直接进老年代。如果业务确实有大量大对象，考虑用 G1，它对大对象的处理更友好。

**GC 停顿时间太长**

如果用的是 Parallel 收集器，停顿长是正常的，它本来就是为吞吐量设计的。想要低延迟就换 G1 或者 ZGC。

如果已经用了 G1 还是停顿长，检查一下 Mixed GC 是不是回收的 Region 太多了，可以调小 -XX:G1MixedGCCountTarget 让每次 Mixed GC 少回收一些 Region。

## 6、常用JVM配置参数

JVM 配置参数分为三大类：内存配置、GC 配置和诊断配置。生产环境最常用的就那么十来个。

### 1）内存配置参数：

1）-Xms 和 -Xmx：堆内存的初始值和最大值，生产上一般设成一样，比如 -Xms4g -Xmx4g，避免运行时动态扩容带来的性能抖动
2）-Xss：每个线程的栈大小，默认 1MB，递归深的业务可能需要调大，比如 -Xss2m
3）-XX:MetaspaceSize 和 -XX:MaxMetaspaceSize：元空间大小，类加载多的应用要关注，比如用了大量动态代理的系统

### 2）GC 配置参数：

1）-XX:+UseG1GC：启用 G1 收集器，Java 9+ 默认就是 G1
2）-XX:+UseZGC：启用 ZGC，追求低延迟的场景，Java 15+ 推荐
3）-XX:MaxGCPauseMillis=200：期望的最大 GC 停顿时间，G1 会尽量满足但不保证

### 3）诊断配置参数：

1）-XX:+HeapDumpOnOutOfMemoryError：OOM 时自动 dump 堆，排查内存泄漏必备
2）-XX:HeapDumpPath=/path/to/dump：指定 dump 文件路径
...
> **内存参数的设置原则**
>
> 堆内存不是越大越好。堆太大会导致 GC 时间变长，比如一个 32GB 的堆，Full GC 可能要十几秒。一般经验是：
>
> 1) 普通业务系统 4-8GB 够用
> 2) 大数据处理、缓存密集型应用可以 16-32GB
> 3) 超过 32GB 要考虑是不是架构有问题，能不能拆分服务
>
> **-Xms 和 -Xmx 设成一样大有什么好处？为什么很多人推荐这么做？**
>
> 主要是避免堆动态扩容带来的性能波动。JVM 堆从 Xms 扩容到 Xmx 的过程中，可能触发 Full GC，而且每次扩容都要向操作系统中申请内存，这个过程有开销。设成一样大，JVM 启动时就把内存一次性分配好，运行时不调用扩容，GC 行为更稳定。缺点是启动慢一点，物理内存占用固定，但对生产环境来说这点代价可以接受。
