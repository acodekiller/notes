...
一个操作被拆成多步执行，中间可能被其他线程插一脚。比如 i++ 实际上是 load、add、store 三条指令，任何一步之间都可能被打断。

- ***缓存*导致的可见性问题；**

线程 A 改了共享变量，线程 B 可能看不到。因为 A 改完可能还在自己的工作内存里没刷到主内存，或者 B 读的是自己工作内存里的旧缓存。

- ***编译优化*带来的有序性问题。**

编译器和 CPU 为了优化性能会对指令重排序，单线程下没问题，多线程下可能看到不符合预期的执行顺序。

> **指令重排发生在三个层面**：
>
> 1）编译器重排：Java 编译器在生成字节码时，会根据优化策略调整代码顺序。比如把两条不相关的赋值语句调换位置，让寄存器分配更高效。
>
> 2）CPU 重排：现代 CPU 都是乱序执行的，内部有多条流水线并行处理指令。没有数据依赖的指令会被并行执行，完成后再按原始顺序提交结果。
>
> 3）内存系统重排：CPU 有 store buffer 和 invalidate queue，写操作可能先进 buffer 再刷到缓存，读操作可能从 queue 里拿到旧数据。从其他 CPU 视角看，内存操作的顺序和实际执行的顺序不一样。
>
> **提问：as-if-serial 和 happens-before 有什么区别？**
>
> 回答：
> as-if-serial 是单线程语义保证，不管怎么重排，单线程执行结果不变。happens-before 是多线程语义保证，定义了操作之间的可见性关系。编译器和 CPU 在重排时两个都要遵守，as-if-serial 保证单线程正确，happens-before 保证多线程正确。

## 5、并行和并发有什么区别

| 特性         | 并发                               | 并行                     |
| :----------- | :--------------------------------- | :----------------------- |
| **核心**     | 处理多个任务的能力                 | 同时执行多个任务         |
| **物理时间** | 宏观同时，微观串行                 | 真正的同时               |
| **硬件基础** | 单核 CPU 即可实现                  | 必须多核 CPU 或多台机器  |
| **关注点**   | 任务结构、调度、交替执行           | 任务拆分、负载均衡、加速 |
| **类比**     | 一个人同时吃面和打电话（快速切换） | 两个人各做一件事         |
...
| **资源拥有**  | 拥有独立的地址空间、堆、栈、文件描述符等              | 共享进程的资源（代码、数据、文件等），拥有独立的栈和寄存器 |
| **内存隔离**  | 进程间内存相互隔离，一个崩溃不影响其他                | 同一进程内线程共享内存，一个线程崩溃可能导致整个进程崩溃   |
| **切换开销**  | **大**（需要切换页表、刷新 TLB、切换地址空间）        | **小**（只需切换程序计数器、寄存器和栈）                   |
| **通信方式**  | 复杂（需要 IPC：管道、消息队列、共享内存、Socket 等） | 简单（直接读写共享变量，注意加锁）                         |
| **创建/销毁** | 慢、开销大                                            | 快、开销小（约比进程轻量10-100倍）                         |
| **依赖关系**  | 独立，不依附于其他进程                                | 必须依附于某个进程，线程不能孤立存在                       |
| **系统调用**  | `fork()`、`exec()`、`wait()`                          | `pthread_create()`、`clone()`                              |
...
[Future、FutureTask和CompletableFuture的区别](/notes/java/java并发编程/Future&FutureTask&CompletableFuture的区别)
...
首先得说一下同步队列、等待队列这些东西，我们会把我们想要互斥使用的资源定义到object类里面去，当线程想要获得这个资源时，它得先去获得这个资源的锁，这种资源是互斥访问的，同一时间只能有一个线程去获得这个资源的锁，并拿到这个锁去访问这个资源，而正在等待这个资源的线程会被放到等待队列中，正在竞争这个资源的线程会被放到同步队列中。

而当这个资源想让正在访问这个资源的线程阻塞起来时，就可以调用wait方法，那么当前线程就会被阻塞起来，正在同步队列中的线程就可以去竞争获得这把锁，只有拿到这把锁的线程才能去访问这个资源，而当我们调用notify方法时，它会随机唤醒等待队列中的线程，将它放入到同步队列中，并让同步队列中的队列去争夺锁，而notifyAll会唤醒等待队列中的所有线程，将他们都放入到同步队列中，让所有在同步队列中的线程去争夺锁。

这些通信方法有一个共同的特点：就是*通过资源来通知线程通信，他们都必须获得锁的线程才会去执行该方法。*而sleep和yield方法是当前线程主动执行的，所以他们被定义在Thread类中，sleep会使当前线程睡眠，但不会释放占有资源的锁，而yield方法会给优先级大于等于它的线程优先执行，如果他们不是静态方法，那么还需要new一个Thread出来，那就不是让当前线程进行sleep或yield了。
...
多个线程同时被阻塞，它们中的一个或者全部都在等待某个资源被释放。由于线程被无限期地阻塞，因此程序不可能正常终止。它的四个必要条件：

- **互斥**：所谓互斥就是进程在某一时间内独占资源。
- **不可剥夺**：进程已获得资源，在末使用完之前，不能强行剥夺。
- **请求并等待**：一个进程因请求资源而阻塞时，对已获得的资源保持不放。
...
## 28、shutdown和shutdownNow有什么区别

shutdown() 是温柔地关门：不再接受新任务，但队列里排队的任务和正在执行的任务都会跑完。线程池进入 SHUTDOWN 状态，等待所有任务执行完毕才真正终止。

shutdownNow() 是粗暴地拉闸：不接受新任务，尝试中断正在执行的任务，队列里没执行的任务直接打包返回给你。线程池进入 STOP 状态，但要注意，“尝试中断”不等于“一定能停”，任务能不能被中断取决于它自己有没有响应中断信号。

> shutdown 和 shutdownNow 的执行流程对比：
>
> - shutdown(): 拒绝新任务 → 等待队列任务执行 → 等待执行中任务完成 → TERMINATED
> - shutdownNow(): 拒绝新任务 → 中断执行中任务 → 清空队列并返回 → TERMINATED

## 29、Interrupt

线程打断是Java中用于**优雅停止线程**的协作机制，不是强制杀死线程。

### 1）核心方法

- `thread.interrupt()`：设置线程的中断标志为 `true`
- `thread.isInterrupted()`：检查中断标志（不清除）
- `Thread.interrupted()`：检查中断标志并**清除**（还原为false）
    ...
    public static void main(String[] args) throws InterruptedException {
        Thread task = new Thread(() -> {
            ...
            for (int i = 0; i < 1_000_000_000 && !Thread.currentThread().isInterrupted(); i++) {
                if (i % 100_000_000 == 0) {
                    System.out.printf("已执行 %d 次计算...%n", i);
                }
            }
            
            long cost = System.currentTimeMillis() - start;
            System.out.printf("结束执行，耗时 %d ms%n", cost);
        });
...
当线程处于 `sleep()`、`wait()`、`join()`、`LockSupport.park()` 等阻塞状态时，`interrupt()` 会**抛出 `InterruptedException`**。

### 4）实践

```java
public class CustomInterruptDemo {
    ...
    static class DownloadTask implements Runnable {
        ...
        public void run() {
            ...
            for (int progress = 0; running && progress <= 100; progress += 10) {
                try {
                    Thread.sleep(500);  // 模拟下载过程
                } catch (InterruptedException e) {
                    // 被中断时，优雅退出
                    System.out.println("下载被中断，当前进度: " + progress + "%");
            ...
            if (Thread.currentThread().isInterrupted()) {
                System.out.println("执行中断清理：删除临时文件");
            } else {
...
![image-20260508150419720](imgs/image-20260508150419720.png)

1. **背景**：旧版`synchronized`直接使用重量级锁（操作系统mutex），用户态到内核态切换开销大。但实际场景中大部分锁竞争并不激烈，甚至只有单线程使用。所以Java 6以后引入了锁升级机制，让锁根据竞争程度自动升级。

2. **四种状态**：无锁 → 偏向锁 → 轻量级锁 → 重量级锁，**单向升级，不能降级**。

   - **偏向锁**：记录线程ID，同一线程再次加锁只需比较ID，开销极小。（再次进入的时候会先判断threadid是否与其线程id一致，如果一致则可以直接使用此对象）

   - **轻量级锁**：多线程交替执行时，使用CAS自旋，不阻塞线程，避免上下文切换。

   - **重量级锁**：竞争激烈时，升级为mutex，未获得锁的线程阻塞。

3. **存储位置：**锁状态记录在Java对象的**Mark Word**（对象头）中，包括标志位、线程ID、锁记录指针等。

![image-20260422180902580](imgs/image-20260422180902580.png)

**Mark Word 在不同锁状态下的结构：**

| 锁状态   | 25 bits                      | 4 bits   | 1 bit(偏向) | 2 bits(标志位) |
| :------- | :--------------------------- | :------- | :---------- | :------------- |
| 无锁     | 对象的hashCode               | 分代年龄 | 0           | 01             |
| 偏向锁   | 线程ID(23bits)               | epoch    | 1           | 01             |
| 轻量级锁 | 指向栈中锁记录的指针(30bits) |          |             | 00             |
| 重量级锁 | 指向互斥量的指针(30bits)     |          |             | 10             |
| GC标记   | 空                           |          |             | 11             |

4. **升级过程**：
   1. 无锁 → 偏向锁：第一次加锁时，Mark Word写入当前线程ID。
   2. 偏向锁 → 轻量级锁：另一线程竞争时，在安全点撤销偏向锁，升级为轻量级锁，线程通过CAS尝试获取锁。
   3. 轻量级锁 → 重量级锁：自旋超过一定次数（或自适应自旋判断失败），升级为重量级锁，线程阻塞。

> 偏向锁在Java 15后默认关闭，因为高并发场景下偏向锁撤销本身也有开销。但了解这个机制有助于我们写出更优的并发代码，比如尽量缩短同步块、减少锁竞争，让锁尽量停留在轻量级状态。
...
```shell
#1：mem = allocate();		//分配内存空间:在堆中开辟一块足够大的内存，此时内存中的值都是默认值（0、null等）
#2：ctorSingleton(mem);		//初始化对象:调用构造函数，设置成员变量的初始值
#3：instance = mem;			//建立连接:将instance引用指向这块内存地址
```
...
- Lock支持非公平锁(默认)和公平锁，而synchronized不支持。（公平锁：线程按照**请求锁的时间顺序**排队，前面的线程先获得锁，与优先级无关）
...
线程想获取资源，先CAS改state，成功了就拿到资源，失败了就包装成Node节点加入队列尾部，然后park挂起等着。前面的线程释放资源后会unpark唤醒队列里的后继节点。

![image-20260508104432148](imgs/image-20260508104432148.png)

### 3）工作流程

![image-20260508104553012](imgs/image-20260508104553012.png)

AQS 把排队、阻塞、唤醒这些脏活累活都封装好了，子类只需要实现 `tryAcquire`、`tryRelease` 这几个方法，告诉 AQS 什么时候拿到资源、什么时候释放资源就行。

### 4）两种模式

1) 独占模式：同一时刻只有一个线程能持有资源。ReentrantLock 就是独占模式，state 为 0 表示没被占用，大于 0 表示被某个线程占有。

2) 共享模式：多个线程可以同时持有资源。Semaphore 允许最多 N 个线程同时访问，state 初始值设为 N，每个线程获取时 state 减 1，释放时加 1，减到 0 就没许可了。

### 5)  AQS为什么用CLH队列变体而不是普通队列

1. 原版 CLH 在高竞争长等待场景下会浪费大量 CPU 周期。AQS 将自旋替换为 **`LockSupport.park()`** 阻塞，当锁被释放时，头节点会主动 **`unpark`** 唤醒后继线程。
2. 改成阻塞后，被阻塞的线程无法再“主动观察前驱状态”。为了让前驱释放锁时能唤醒后继，必须能快速找到 next 节点。因此 AQS 引入了双链表（prev + next），并保证入队 / 出队 / 取消时维护指针一致性。
3. 节点取消的时候要从队列里摘掉，单向链表只能从头遍历找前驱，双向链表直接通过 prev 指针就能找到前驱，时间复杂度从 O(n) 降到 O(1)。
...
ReentrantLock 底层就是 AQS + CAS 那套东西。核心是一个 int 类型的 state 变量和一个等待队列。

加锁的时候，线程用 CAS 尝试把 state 从 0 改成 1，改成功了就拿到锁，把 exclusiveOwnerThread 设成自己。改失败了说明锁被别人占着，乖乖去同步队列尾部排队，然后 park 挂起。

解锁的时候，state 减 1，如果减到 0 就唤醒队列里的第一个等待线程。

可重入的实现很简单：同一个线程再次加锁，发现 exclusiveOwnerThread 就是自己，直接把 state 加 1，不用排队。释放的时候每次减 1，减到 0 才真正释放锁。

### 2）源码分析

在内部定义了一个Sync类并继承了AQS，自定义了`tryAcquire`、`tryRelease`、`isHeldExcluesively`

![image-20260508113119980](imgs/image-20260508113119980.png)

#### 1. 非公平

- nonfairTryAcquire方法：

![image-20260508113326444](imgs/image-20260508113326444.png)

- tryRelease方法：

![image-20260508113457989](imgs/image-20260508113457989.png)
...
![image-20260508113646659](imgs/image-20260508113646659.png)

再看下ReentrantLock尝试加锁，如果未抢到则入队的过程：

![image-20260508113718058](imgs/image-20260508113718058.png)
...
`StampedLock` 是 Java 8 引入的一种新的锁机制，位于 `java.util.concurrent.locks` 包中。它提供了三种锁模式：**写锁、读锁（悲观读）、乐观读**，性能上比 `ReentrantReadWriteLock` 更优。

### 1）三种模式

| 模式                          | 说明                                           | 是否阻塞         | 适用场景                     |
| :---------------------------- | :--------------------------------------------- | :--------------- | :--------------------------- |
| **写锁（WriteLock）**         | 独占锁，类似 `ReentrantReadWriteLock` 的写锁   | 阻塞其他读/写    | 修改数据                     |
| **读锁（ReadLock）**          | 悲观读锁，类似 `ReentrantReadWriteLock` 的读锁 | 阻塞写，不阻塞读 | 读多写少，且不能容忍脏读     |
| **乐观读（Optimistic Read）** | 不加锁，读前获取戳记，读后验证                 | 不阻塞任何锁     | 读多写极少，可以容忍偶尔重读 |

### 2）特点

- 不可重入：与 ReentrantLock 不同，不能重复获取同一把锁 
- 性能优越：乐观读完全无锁，性能极高 
- 不支持条件变量：不支持 Condition（没有 newCondition() 方法） 
- 支持锁转换：可以在读锁/写锁之间转换 
- 非公平锁：不是公平锁（默认）

### 3）内部实现原理

`StampedLock` 用一个 64 位的 long 值来管理状态。低 7 位表示读锁的持有数量，第 8 位是写锁标志位，高 56 位是版本号。每次写锁释放，版本号就加 1。

乐观读的 validate 方法其实就是对比当前版本号和拿到的 stamp 是否一致，不一致说明中间有写操作发生过。

![image-20260507143547974](imgs/image-20260507143547974.png)

### 4）注意事项

- 不可重入：同一个线程拿着写锁再去拿写锁会直接死锁，这点和 ReentrantLock 不一样
...
public void deadlock() {
    ...
    // 再次获取写锁 → 死锁！
    long stamp2 = lock.writeLock();  // 线程会永远阻塞
}
...
- 中断问题：如果线程用 writeLock 或 readLock 等待获取锁时被 interrupt，会导致 CPU 飙升。解决办法是用 readLockInterruptibly 或 writeLockInterruptibly
- 不支持 Condition：如果业务需要等待通知机制，老老实实用 ReentrantLock
...
> **提问：为什么 StampedLock 不支持可重入？**
>
> 回答：设计上的取舍。StampedLock 用一个 long 值管理所有状态，要支持可重入就得记录每个线程的输入次数，要么用 ThreadLocal 要么用额外的数据结构，会增加复杂度和开销。Doug Lea 认为对于大多数场景，不可重入是可以接受的，换来的是更简洁的实现和更好的性能。
>
> **提问：高并发写的场景下 StampedLock 表现怎么样？**
>
> 回答：表现一般，甚至不如 synchronized。因为乐观读老是校验失败，每次都要降级成悲观读重试，白白浪费 CPU。而且 StampedLock 的写锁竞争激烈时也没什么优势。所以写多的场景还是用 ReentrantLock 或者 synchronized，别硬上 StampedLock。
...
但ThreadLocal的实例以及其值并不是存放在栈上的。因为ThreadLocal实例实际上也是被其创建的类持有（更顶端应该是被线程持有），而ThreadLocal的值其实也是被线程实例持有，它们都是位于堆上，只是通过一些技巧将可见性修改成了线程可见。
...
ThreadLocalMap中使用的key为ThreadLocal的弱引用，而value是强引用。

所以，如果ThreadLocal没有被外部强引用的情况下，在垃圾回收的时候，key会被清理掉，而value不会被清理掉。

这样一来，ThreadLocalMap中就会出现key为null的Entry。假如我们不做任何措施的话，value永远无法被GC回收，这个时候就可能会产生内存泄露。

ThreadLocalMap实现中已经考虑了这种情况，在调用set()、get()、remove()方法的时候，会清理掉key为null的记录。使用完ThreadLocal方法后最好手动调用remove()方法。

#### 2. 解决方案

每次使用完ThreadLocal，都调用它的remove()方法，清除数据。

#### 3. 为什么ThreadLocalMap的key要设计成弱引用？(待删)

不妨反过来想想，如果使用强引用，当`ThreadLocal`对象（假设为`ThreadLocal@123456`）的引用（即：TL_INT，是一个强引用，指向`ThreadLocal@123456`）被回收了，`ThreadLocalMap`本身依然还持有`ThreadLocal@123456`的强引用，如果没有手动删除这个key，则`ThreadLocal@123456`不会被回收，所以只要当前线程不消亡，ThreadLocalMap引用的那些对象就不会被回收，可以认为这导致Entry内存泄漏。
...
> **问：LinkedBlockingQueue两把锁是怎么实现的**
>
> 回答：它用了 takeLock 和 putLock 两把锁，take 操作只加 takeLock，put 操作只加 putLock。关键在于用了一个 AtomicInteger 类型的 count 来记录元素数量，这个是线程安全的。入队时先加 putLock，操作完 count.incrementAndGet()；出队时加 takeLock，操作完 count.decrementAndGet()。两边操作的是链表的不同端，加上原子计数器，就不会冲突。

### 3)   使用场景

- Java 5 之前实现同步存取时，可以使用普通的一个集合，然后在使用线程的协作和线程同步可以实现生产者，消费者模式，主要的技术就是用好，wait,notify,notifyAll,sychronized 这些关键字。而在 java 5 之后，可以使用阻塞队列来实现，此方式大大简少了代码量，使得多线程编程更加容易，安全方面也有保障。
- `BlockingQueue`接口是Queue的子接口，它的主要用途并不是作为容器，而是作为线程同步的的工具，因此他具有一个很明显的特性，当生产者线程试图向`BlockingQueue`放入元素时，如果队列已满，则线程被阻塞，当消费者线程试图从中取出一个元素时，如果队列为空，则该线程会被阻塞，正是因为它所具有这个特性，所以在程序中多个线程交替向`BlockingQueue`中放入元素，取出元素，它可以很好的控制线程之间的通信。
...
阿里巴巴规范规定不允许使用Executors去创建，而是通过`ThreadPoolExecutor`的方式，这样的处理方式让写的同学更加明确线程池的运行规则，规避资源耗尽的风险。

- `newFixedThreadPool`和`newSingleThreadExecutor`：  主要问题是堆积的请求处理队列可能会耗费非常大的内存，甚至OOM。  
- `newCachedThreadPool`和`newScheduledThreadPool`：  主要问题是线程数最大数是`Integer.MAX_VALUE`，可能会创建数量非常多的线程，甚至OOM。  

### 2)   ThreadPoolExecutor中的参数

> 当新任务通过 `execute` 提交时，判断顺序如下：
>
> 1. 如果当前运行的线程数 **小于** `corePoolSize`，则创建新线程执行任务。
> 2. 否则，尝试将任务放入 `workQueue`：
>    - 如果入队成功，等待核心线程空闲后处理。
>    - 如果入队失败（队列已满），且当前线程数 **小于** `maximumPoolSize`，则创建新的**非核心线程**执行任务。
>    - 如果队列已满且线程数已达到 `maximumPoolSize`，则触发**拒绝策略**。
...
CountDownLatch 底层基于 AQS 的共享模式实现。计数器值就是 AQS 的 state。

countDown() 通过 CAS 将 state 减 1，减到 0 时调用 doReleaseShared() 唤醒等待队列中的所有线程。 await() 检查 state 是否为 0，不为 0 就入队阻塞。

关键点：state 只能从初始值往下减，减到 0 后不能重置，没有提供任何 public API 将其重置回 N。这就是 CountDownLatch 不可复用的根本原因。

### 3）注意

1) 计数器只能用一次，归零后再调用 await() 不会阻塞，直接通过

2) countDown() 调用次数超过初始值不会报错，但没有任何效果

3) 如果某个子任务异常退出没调用 countDown()，主线程会永远阻塞。生产环境要用 await(timeout, unit) 加超时保护

4) await() 可以被中断，中断后抛 InterruptedException，计数器值不变
...
CyclicBarrier 基于 ReentrantLock 和 Condition 实现，不依赖 AQS 的共享模式。

内部维护一个 count 计数器和一个 generation 代。线程调用 await() 时先获取锁，然后 count 减 1。如果减完不等于 0，调用 condition.wait() 阻塞当前线程。如果减完等于 0，说明最后一个线程到达，先执行 barrierAction 回调，再调用 condition.signalAll() 唤醒所有等待线程，最后重置 count 并更新 generation 开启下一轮。
...
1) parties 数量和实际线程数必须匹配，少了会永远阻塞，多了也永远阻塞

2) barrierAction 由最后一个到达的线程执行，执行期间其他线程还在阻塞，别在里面搞耗时操作

3) 屏障被打破后必须 reset 才能复用，否则后续 await 直接抛 BrokenBarrierException

### 5）CountDownLatch和CyclicBarrier的区别

`CountDownLatch`与`CyclicBarrier`都是用于控制并发的工具类，都可以理解成维护的就是一个计数器，但是这两者还是各有不同侧重点的：

（1）`CountDownLatch`一般用于某个线程A等待若干个其他线程执行完任务之后，它才执行；而`CyclicBarrier`一般用于一组线程互相等待至某个状态，然后这一组线程再同时执行；`CountDownLatch`强调一个线程等多个线程完成某件事情。`CyclicBarrier`是多个线程互等，等大家都完成，再携手共进。
...
（4）`CountDownLatch`是不能复用的，而`CyclicBarrier`是可以复用的。`CyclicBarrier`减为0后会重新变为初始设置的值，而`CountDownLatch`减为0后一直为0。

## 3.  Semaphore（信号量）

### 1）定义

`synchronized`和`ReentrantLock`都是一次只允许一个线程访问某个资源，`Semaphore`(信号量)可以指定多个线程同时访问某个资源。（类似与停车位。）

### 2）原理

底层实现依赖 AQS，许可数就是 AQS 的 state 值。acquire() 对应 AQS 的共享式获取，通过 CAS 操作扣减 state；release() 则是 CAS 增加 state 并唤醒等待队列中的线程。

![image-20260507144753936](imgs/image-20260507144753936.png)
...
Semaphore 构造函数可以指定是否公平：`new Semaphore(5, true)` 表示公平模式。

公平模式下，等待时间最长的线程优先获得许可，底层用 FIFO 队列排队。好处是不会饿死任何线程，坏处是吞吐量下降，因为每次释放许可都要从队列头唤醒线程，多了一次上下文切换。

非公平模式下，新来的线程可以直接抢许可，如果刚好有许可就拿走，不用排队。吞吐量高，但可能有线程一直抢不到。生产环境大部分场景用默认的非公平模式就够了，除非对公平性有明确要求，比如任务必须按提交顺序执行。

### 5）注意

1) acquire() 和 release() 必须成对出现，漏了 release() 会导致许可永久丢失，其他线程永远阻塞

2) release() 没有校验当前线程是否持有许可，调用多次会凭空增加许可数，破坏限流语义

3) tryAcquire() 返回 false 时别忘了处理，不能当没事发生直接往下走

## 4、Exchanger 

`Exchanger` 是 Java 并发包（`java.util.concurrent`）中的一个同步工具类，专门用于**两个线程之间交换数据**。

### 1）概念

`Exchanger` 提供了一个同步点，两个线程可以在此**交换彼此的数据**：
...
- 配对数量：必须恰好两个线程
- 阻塞行为：第一个到达的线程会阻塞，等待第二个
- 数据交换：双方同时将数据交给对方
- 对称性：两个线程的角色完全对等
- 可重用：同一对 Exchanger 可以被多对线程重复使用
    ...
    public static void main(String[] args) {
        ...
        Thread threadA = new Thread(() -> {
            try {
                ...
                System.out.println("线程A 准备交换: " + dataA);
                
                String receivedFromB = exchanger.exchange(dataA);
                System.out.println("线程A 收到: " + receivedFromB);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        });
        ...
        Thread threadB = new Thread(() -> {
            try {
                ...
                System.out.println("线程B 准备交换: " + dataB);
                
                String receivedFromA = exchanger.exchange(dataB);
                System.out.println("线程B 收到: " + receivedFromA);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        });
...
`CompletableFuture` 是 Java 8 引入的异步编程工具，解决了老版 Future 的两个痛点：一是 get 方法会阻塞线程，二是没法把多个异步任务串起来。

`CompletableFuture` 支持链式调用，可以用 `thenApply`、`thenCompose` 这些方法把多个异步操作像流水线一样串起来，前一个任务完成自动触发下一个，不用手动阻塞等结果。

基本用法是这样的：

1）用 `supplyAsync` 提交一个有返回值的异步任务，或者 `runAsync` 提交一个无返回值的任务

2）用 `thenApply` 对结果做转换，`thenAccept` 消费结果，`thenRun` 单独执行后续动作

3）用 `exceptionally` 或 `handle` 处理异常

4）用 `thenCombine` 合并两个任务的结果，用 `allOf` 等待多个任务全部完成
...
![image-20260507151946462](imgs/image-20260507151946462.png)

### 2）与Future的区别

Future 是 Java 5 引入的，只能通过 get 阻塞拿结果，没法链式组合，异常处理也很麻烦。

| 特性       | Future           | CompletableFuture              |
| ---------- | ---------------- | ------------------------------ |
| 获取结果   | get 阻塞         | 回调，不阻塞                   |
| 链式调用   | 不支持           | 支持                           |
| 组合多任务 | 手动循环         | allOf/anyOf                    |
| 异常处理   | try-catch 包 get | exceptionally/handle           |
| 手动完成   | 不支持           | complete/completeExceptionally |

### 3）线程池的选择

默认情况下` CompletableFuture` 用的是 `ForkJoinPool.commonPool`，这个池子是整个 JVM 共享的，核心线程数等于 CPU 核心数减 1。如果任务里有 IO 操作或者阻塞调用，很容易把公共池打满，影响其他用到 `commonPool` 的地方。
...
1) 忘了处理异常：如果不加 exceptionally 或 handle，异常会被吞掉，调试的时候一脸懵

2) get 超时：虽然 CompletableFuture 主打不阻塞，但有时候还是得等结果。用 get(timeout, unit) 而不是 get()，避免无限等待

3) 回调阻塞：在 thenApply 这些回调里搞阻塞操作，会拖慢整个线程池。如果必须阻塞，用 thenApplyAsync 把后续操作扔到独立线程池

4) allOf 拿不到结果：allOf 返回的是 CompletableFuture<Void>，拿结果得用 join 逐个取
...
1) completeOnTimeout: 超时自动用默认值完成  
2) orTimeout: 超时抛 TimeoutException  
3) copy: 创建一个不能被外部 complete 的副本
...
> **提问：thenApply 和 thenApplyAsync 有什么区别？**
>
> 回答：thenApply 用的是上一个任务的执行线程，如果上一个任务已经完成，就用当前调用 thenApply 的线程。thenApplyAsync 会把后续操作提交到线程池，默认是 commonPool，也可以自定义线程池。如果回调里有耗时操作，用 Async 版本避免阻塞。
>
> ---
>
> **提问：怎么实现多个异步任务，任意一个成功就返回？**
>
> 回答：用 anyOf，但它有个问题，异常也算完成。如果想要第一个成功的结果，得自己写逻辑，用一个 CompletableFuture 作为结果容器，每个任务完成时调用 complete，因为 complete 只有第一次调用生效，后面的会被忽略。
>
> ---
>
> **提问：CompletableFuture 的 join 和 get 有什么区别？**
>
> 回答：功能一样都是阻塞拿结果，区别是 get 抛的是受检异常 ExecutionException 和 InterruptedException，必须 try-catch 或者 throws。join 抛的是非受检异常 CompletionException，代码写起来简洁一些。在 Stream 的 lambda 里用 join 更方便，不用处理受检异常。
>
> ---
>
> **提问：生产环境用 CompletableFuture 有什么注意事项？**
>
...
ForkJoinPool 是 Java 7 引入的一个专门处理分治任务的线程池。核心思想是把一个大任务递归拆成小任务，小任务并行执行完再把结果合并起来。

它和普通线程池最大的区别是工作窃取算法：每个工作线程都有自己的双端队列，自己产生的子任务从队列头部取，空闲的时候会从别的线程队列尾部偷任务过来执行。这样能把 CPU 资源榨得比较干净，不会出现有的线程忙死、有的线程闲着的情况。

ForkJoin 的执行流程：

1) Fork 阶段：大任务递归拆分成小任务，每个子任务提交到当前线程的工作队列

2) Compute 阶段：小任务在各个线程并行执行

3) Join 阶段：等待子任务完成，把结果合并起来，逐层向上汇总

![image-20260507172711347](imgs/image-20260507172711347.png)
...
Long result = pool.invoke(new SumTask(1, 10_000_000));

```

### 3）原理

每个工作线程维护一个双端队列 WorkQueue。自己 fork 出来的子任务从队列头部压入，执行时也从头部取，这是 LIFO，能利用 CPU 缓存局部性。

别的线程来偷任务时，从队列尾部取，这是 FIFO。偷大任务效率更高，因为大任务还能继续拆分，不用频繁偷。

![image-20260507172837991](imgs/image-20260507172837991.png)

### 4）和普通线程池的区别

| 特性       | ForkJoinPool         | ThreadPoolExecutor |
| ---------- | -------------------- | ------------------ |
| 任务队列   | 每个线程一个双端队列 | 共享一个阻塞队列   |
| 调度策略   | 工作窃取             | 中央调度           |
| 适合场景   | 递归分治任务         | 独立的并行任务     |
| 任务类型   | ForkJoinTask         | Runnable/Callable  |
| 默认线程数 | CPU 核心数           | 需要手动指定       |

Java 8 引入了 ForkJoinPool.commonPool()，这是一个 JVM 全局共享的 ForkJoinPool 实例，并行流和 CompletableFuture 默认都用它。

commonPool 的线程数默认是 CPU 核心数减 1，可以通过 JVM 参数调整：

```shell
-Djava.util.concurrent.ForkJoinPool.common.parallelism=16
```

注意：因为是全局共享的，如果在 commonPool 里执行阻塞操作，会影响其他用到 commonPool 的代码，比如并行流会变慢。

### 5）RecursiveTask 和 RecursiveAction

ForkJoinTask 有两个常用子类：

1) **RecursiveTask**：有返回值，compute 方法返回计算结果，适合累加、归并这类需要合并结果的场景  
2) **RecursiveAction**：无返回值，适合并行遍历、批量更新这类不需要合并结果的场景

> 提问：为什么 ForkJoinPool 默认线程数是 CPU 核心数而不是核心数的两倍？ 
>
> 回答：ForkJoinPool 设计目标是处理计算密集型任务，这种任务基本不阻塞，线程数等于 CPU 核心数就能把 CPU 跑满。线程太多反而有上下文切换开销。普通线程池建议核心数两倍是因为假设任务有一半时间在等 IO，线程多一些能在 IO 等待时让 CPU 有活干。
>
> ---
>
> 提问：fork 和 join 的顺序有讲究吗？ 
>
> 回答：有讲究。推荐写法是 left.fork() 然后 right.compute() 然后 left.join()。right.compute() 让当前线程直接干活而不是也 fork 出去，省一次任务调度。如果两边都 fork 再 join，当前线程就干等着了，浪费资源。另外 join 要在 fork 之后调用，不然会死锁。
>
> ---
>
> 提问：并行流用的是 commonPool，怎么让它用自定义线程池？ 
>
> 回答：两种办法。一是把并行流操作包在自定义 ForkJoinPool 的 submit 里执行，这样流操作会用提交时所在的池。二是用 JVM 参数调整 commonPool 的并行度，但这会影响全局。生产环境如果担心并行流阻塞 commonPool 影响其他模块，建议用第一种方式隔离。