...
### 15）为什么HashMap中MAXIMUM_CAPACITY是2^30

**1. 核心原因：避免超出 `int` 范围变成负数**

`HashMap` 中的容量（`capacity`）是用 `int` 类型存储的。

- `2^31 - 1` (2,147,483,647) 是 `Integer.MAX_VALUE`，是一个合法的正数。
- `2^31` (2,147,483,648) 已经超过了 `int` 的范围，会溢出变成 `-2,147,483,648`。

`HashMap` 中有很多位运算来求哈希索引，比如 `(n - 1) & hash`。如果容量是负数，`n-1` 就会变成负数，导致按位与的结果出现负的数组下标，直接引发 `ArrayIndexOutOfBoundsException`。

**2. 位运算的限制：需要支持 `n-1` 为全 1**

`HashMap` 要求容量必须是 2 的幂次方。对于 2 的幂 `n`，`n-1` 的二进制是低位全 1，用于按位与运算。

### 16）jdk1.7和1.8中map实现的区别

**1. 数据结构**

jdk1.7：数据+链表，链表较长时，查找、插入和删除操作的时间复杂度会退化为O(n)；

jdk1.8：数据+链表/红黑树，当链表的长度大于8且数组长度大于64时，链表会转换为红黑树，红黑树的查找、插入和删除操作的时间复杂度会退化为O(logn)

**2. hash冲突的解决方式**

jdk1.7：单纯使用链表解决哈希冲突，新元素总是插入到链表头部。在多线程环境下，如果发生扩容，可能会形成环形链表，导致死循环。

jdk1.8：除了链表，在满足条件时使用红黑树解决哈希冲突。并且在链表插入元素时，新元素会插入到链表尾部，避免了 Java 1.7 中在多线程扩容时可能出现的环形链表问题。

**3. 扩容机制**

jdk1.7：扩容时，会重新计算每个元素的哈希值，并将其分配到新的数组位置。这个过程比较耗时，因为需要遍历整个链表并重新计算哈希值。

jdk1.8：扩容时，利用了元素哈希值和旧数组容量的关系，减少了重新计算哈希值的次数。通过 `(e.hash & oldCap) == 0` 来判断元素在新数组中的位置，要么在原索引位置，要么在原索引位置 + 旧数组容量的位置。

**4. 计算hash的方式**

jdk1.7： 对键的hashcode进行了一系列复杂的扰动计算，以减少哈希冲突。

jdk1.8：计算方式相对简单，直接取键的 `hashCode()` 高 16 位和低 16 位进行异或运算。

### 17）为什么链表转红黑树的阈值是 8，不是 6 或者 10？

根据泊松分布计算，在负载因子 0.75 的情况下，一个桶里元素个数达到 8 的概率已经非常低了，大概是千万分之六。选 8 是因为这个概率下转红黑树基本只会在 hash 分布极差的时候发生，正常情况几乎不会触发，既保证了极端情况的性能，又避免了频繁转换的开销。
...
-  并发安全：HashMap是非线程安全的，HashTable是线程安全的,因为HashTable内部的方法基本都经过synchronized修饰。
...
private HashEntry<K,V> scanAndLockForPut(K key, int hash, V value) {
    ...
    // 在自旋过程中，会遍历链表查找这个 key
    // 如果遍历到链表末尾都没找到，就会提前创建一个新节点
    if (retries == 0) {
        node = new HashEntry<K,V>(hash, key, value, null); // 预创建
    }
...
- 1.7：只有获得锁的线程能执行`rehash`（扩容）。其他线程被阻塞等待锁，无法参与数据迁移；
- 1.8：多个线程可以同时参与`transfer`（数据迁移），并且正在`put`的线程也可以发现并加入帮助。
