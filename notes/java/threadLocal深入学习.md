...
![在这里插入图片描述](imgs/20210123210723278.png)
...
![在这里插入图片描述](imgs/20210123210812106.png)
...
![在这里插入图片描述](imgs/20210123210856358.png)
...
![在这里插入图片描述](imgs/20210123211111323.png)

#### **（2） 数据准备**

![在这里插入图片描述](imgs/20210123211717292.png)
...
![在这里插入图片描述](imgs/20210124115504330.png)
...
![在这里插入图片描述](imgs/20210124120125266.png)
...
![在这里插入图片描述](imgs/20210124143602480.png)
...
![在这里插入图片描述](imgs/20210124144330949.png)
...
![在这里插入图片描述](imgs/20210124144945153.png)
...
> ---
>
> **提问：为什么 ThreadLocalMap 用线性探测法而不是链地址法？**
>
> 回答：主要是场景决定的。一个线程里 ThreadLocal 的数量通常很少，可能就几个到十几个，冲突概率本来就不高。线性探测法实现简单，缓存友好，连续内存访问比链表的跳转访问快。如果 ThreadLocal 特别多，线性探测效率会下降，但实际场景里几乎不会出现这种情况。
...
普通 ThreadLocal 有个限制：子线程拿不到父线程的值。如果你在主线程 set 了一个值，然后 new 一个子线程去 get，拿到的是 null。

InheritableThreadLocal 解决了这个问题，子线程创建的时候会把父线程的值拷贝一份过去：
...
public class InheritableThreadLocalTest {
    ...
    public static void main(String[] args) {
        ...
        new Thread(() -> {
            String var1 = inheritableThreadLocal.get();
            String var2 = threadLocal.get();
...
但要注意，这只是创建时拷贝一份，后续父子线程各改各的，互不影响。而且用线程池的时候，InheritableThreadLocal 基本没用，因为线程是复用的，不是每次都新建。阿里开源的 TransmittableThreadLocal 专门解决线程池场景的值传递问题。

### 2）原理

提问：InheritableThreadLocal 是怎么实现父子线程传值的？

回答：Thread 对象除了 threadLocals 还有一个 inheritableThreadLocals 字段，也是 ThreadLocalMap 类型。创建子线程的时候，Thread 的构造函数会检查父线程的 inheritableThreadLocals 是否为空，不为空就把父线程的 Map 复制一份给子线程。注意是创建时复制，后续父子线程各改各自的，互不影响。

### 3）注意

提问：线程池场景下 InheritableThreadLocal 有什么问题？

回答：线程池的线程是复用的，不是每次提交任务都新建线程。InheritableThreadLocal 只在线程创建时从父线程拷贝值，复用的线程不会重新拷贝。所以你在主线程 set 了新值，提交到线程池的任务可能拿到的还是上一次的值，甚至是别的任务留下的脏数据。阿里的 TransmittableThreadLocal 就是为了解决这个问题，在任务提交时自动做值传递。
